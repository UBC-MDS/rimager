...
#' @param strength numeric or integer (0 to 1);
#'   the strength of the selected filter effect
#' @param output_path character or NULL (default NULL);
...
imgfilter <- function(input_path, filter_type, strength, output_path = NULL) {
  ...
  if (!is.integer(strength) & !is.numeric(strength)) {
    stop("TypeError: `strength` must be of type integer or numeric.")
  }
  # assert strength is between 0 and 1
  if (strength > 1 | strength < 0) {
    stop("ValueError: 'strength' can only take on values from 0 to 1.")
  }
  ...
  if (filter_type == "blur") {
    ...
    s <- strength / 10
    filt <- array(
      data = 1 / (as.integer(h * s) * as.integer(w * s)),
      dim = c(as.integer(h * s), as.integer(w * s))
    )
  } else {
